...
def authenticateadmin(request):
    ...
    # User exists
    if user is not None and user.username == "raval":
        login(request, user)
...
def signupuser(request):
    ...
    CustomerModel(userid = User.objects.all()[int(lastobject)].id, phoneno = number).save()
    messages.add_message(request, messages.ERROR, "Sign Up, Successfull")
    return redirect('homepage')
...
def declineorder(request, orderpk):
    order = OrderModel.objects.filter(id = orderpk)[0]
    order.status = "Declined, Try again Later"
    order.save()
    return redirect(request.META['HTTP_REFERER'])
